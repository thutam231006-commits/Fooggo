...
@if($order->status === 'pending_payment' && ($order->expires_at || $order->payment_expires_at))<div class="muted">Giữ đến {{ ($order->expires_at ?? $order->payment_expires_at)->format('H:i') }}</div>@endif
