...
@section('content')
@php $statusLabels = ['pending_payment' => 'Chờ thanh toán', 'paid' => 'Chờ căn tin xác nhận', 'preparing' => 'Đang chuẩn bị', 'ready' => 'Sẵn sàng nhận', 'completed' => 'Đã hoàn tất', 'rejected' => 'Căn tin từ chối', 'cancelled' => 'Đã hủy']; @endphp
...
<tr><td><a class="back" href="{{ route('web.orders.show', $order) }}">#{{ $order->id }}</a></td><td>{{ $order->items->sum('quantity') }} phần</td><td>{{ $order->pickup_slot }}</td><td>{{ number_format($order->total, 0, ',', '.') }} đ</td><td><span class="badge">{{ $statusLabels[$order->status] ?? $order->status }}</span>@if($order->status === 'pending_payment' && $order->payment_expires_at)<div class="muted">Giữ đến {{ $order->payment_expires_at->format('H:i') }}</div>@endif @if($order->status === 'rejected' && $order->rejection_reason)<div class="error">{{ $order->rejection_reason }}</div>@endif</td><td>
@if($order->status === 'pending_payment')<div class="actions"><form method="POST" action="{{ route('web.orders.pay', $order) }}">@csrf<input type="hidden" name="redirect_to" value="dashboard"><button class="button" type="submit">Thanh toán</button></form><form method="POST" action="{{ route('web.orders.cancel', $order) }}">@csrf @method('DELETE')<input type="hidden" name="redirect_to" value="dashboard"><button class="button danger" type="submit">Hủy đơn</button></form></div>@else<span class="muted">Xem chi tiết</span>@endif
</td></tr>
